...
zephyr = {
	...
	translator = minetest.get_translator('zephyr'),
}
local S = zephyr.translator
...
local use_rockets = has_tnt and minetest.settings:get_bool(
	"zephyr.use_rockets", true)

local rocket_cooldown = math_min(65000, math_max(1,
	tonumber(minetest.settings:get("zephyr.rocket_cooldown")) or 10))

zephyr.rocket_cooldown = rocket_cooldown

local glider_wear = 0 < glider_uses and (65535 / glider_uses) or nil

zephyr.allow_hangglider_while_gliding = minetest.settings:get_bool(
	"zephyr.allow_hangglider_while_gliding", true)

zephyr.allow_while_hanggliding = minetest.settings:get_bool(
	"zephyr.allow_while_hanggliding", false)
...
minetest.register_chatcommand("zephyrToggleHUD", {
	params = "",
	description = S("Toggle delta-glider's HUD."),
	func = function(name)
		local meta = minetest.get_player_by_name(name):get_meta()
		-- think: zephyr.HUDdisabled
		local value = 0 == meta:get_int("zephyr.HUDd")
		meta:set_int("zephyr.HUDd", value and 1 or 0)
		return true
	end
})
...
function zephyr.register_grounded_check(func)
...
local function shoot_flak_sound(pos)
	minetest.sound_play("zephyr_flak_shot", {
		pos = pos,
...
local function equip_sound(pos)
	minetest.sound_play("zephyr_equip", {
		pos = pos,
		max_hear_distance = 8,
		gain = 1.0,
	}, true)
...
local function player_controls(player)
	...
	local pro = 1 == meta:get_int("zephyr.pro")
	if mouse_controls and keyboard_controls then
		return 0 == meta:get_int("zephyr.keyC"), pro
	else
...
local function update_hud(name, player, rot, rocket_time, speed, vV)
	local info = ""
	if rot and (0 == player:get_meta():get_int("zephyr.HUDd")) then
...
local function damage_glider(player, luaent, crash_damage)
	...
	if stack:to_string() ~= luaent.tool_string then
		...
		for i, is in ipairs(inv:get_list("main")) do
			if is:get_name() == "zephyr:glider" then
				index_alt = i
			end
...
local on_step = function(self, dtime, moveresult)
	...
	if not zephyr.allow_hangglider_while_gliding then
		local luaent
...
local on_use = function(itemstack, player)
	...
	if attach then
		...
		if luaent.name == "zephyr:hangglider" then
		...
		if not zephyr.allow_while_hanggliding then
			for _, obj in ipairs(player:get_children()) do
				luaent = obj:get_luaentity()
				if luaent and luaent.name == "hangglider:glider" then
					return
		...
		pos.y = pos.y + 1.5
		local ent = minetest.add_entity(pos, "zephyr:hangglider")
		if not ent then
...
local function on_place(_, player)
	...
	if keys.aux1 and keys.sneak then
		...
		local pro = 0 == meta:get_int("zephyr.pro")
		meta:set_int("zephyr.pro", pro and 1 or 0)
	...
	then
		...
		-- read and toggle in one line
		local key_c = 0 == meta:get_int("zephyr.keyC")
		meta:set_int("zephyr.keyC", key_c and 1 or 0)
...
minetest.register_entity("zephyr:hangglider", {
	physical = true,
	pointable = false,
	visual = "mesh",
	mesh = "zephyr_hangglider.obj",
	textures = { "zephyr_hangglider.png" },
	static_save = false,
	--Functions
	on_step = on_step,
	grav_speed = 0,
...
minetest.register_tool("zephyr:glider", {
	description = S("Zephyr"),
	inventory_image = "zephyr_glider.png",
	on_use = on_use,
	on_secondary_use = on_place,
})

local mp = minetest.get_modpath("zephyr")
...
minetest.register_alias("glider:glider", "zephyr:glider")
minetest.register_alias("glider:rocket", "zephyr:rocket")
minetest.register_alias("glider:hangglider", "zephyr:hangglider")

print("[zephyr] loaded with"
